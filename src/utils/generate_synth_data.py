...
def sample_pose_init(
    ...
    def sample_pose(obj: bproc.types.MeshObject):
        ...
        while n < 100:
            ...
            obj.set_rotation_euler([0, 0, rot * np.pi / 2 + diff], frame=i)
...
def sample_poses_for_frames(
    ...
    for j in range(dataset_size):
        sample_pose_func = sample_pose_init(
...
def write_data(data: dict):
    ...
    """
    # Clip and convert depth to millimeters
...
for i in range(total_boxes + 1):
    ...
    total_minibatches_overall += math.ceil(combination_size / minibatch_size)

# Outer progress bar for the complete dataset
with tqdm(
    total=total_minibatches_overall, desc="Overall dataset progress", unit="minibatch"
) as pbar_overall:
