...
sys.path.append("/opt/ros/kilted/lib/python3.12/site-packages")

from smart_palletizer_utils.smart_palletizer_utils import utils
...
def gen_boxes(total_boxes: int, n_small_boxes: int) -> List[bproc.types.MeshObject]:
    ...
    for _ in range(1, n_small_boxes + 1):
        obj = bproc.loader.load_obj(OBJ_PATH["small"])
        obj[0].set_scale(np.full((3), 1 / 1000))
        obj[0].set_cp("category_id", utils.CLASS_MAP["small"])
        objs.extend(obj)

    # Generate medium boxes
    for _ in range(n_small_boxes + 1, total_boxes + 1):
        obj = bproc.loader.load_obj(OBJ_PATH["medium"])
        obj[0].set_scale(np.full((3), 1 / 1000))
        obj[0].set_cp("category_id", utils.CLASS_MAP["medium"])
        objs.extend(obj)
...
def sample_pose_func(
    i: int, image_width: int, image_height: int, cam_to_world_transform: np.ndarray
):
    """
    ...
    def sample_pose(obj: bproc.types.MeshObject):
        ...
        while n < 100:
            ...
            z = -np.random.randint(0, 112) * 0.01 - 1.1
            ...
            euler_camera = cam_to_world_transform[:3, :3].T @ euler_camera
            euler_camera = R.from_matrix(euler_camera).as_euler("XYZ")

            # Set object location and rotation at frame i
            obj.set_location(
                (cam_to_world_transform @ np.array([x, y, z, 1]).T)[:3],
                frame=i,
            )
            obj.set_rotation_euler(euler_world, frame=i)

            # Get the bounding box vertices in world coordinates
            bbox_world = obj.get_bound_box()  # Nx3

            # Convert to homogeneous coordinates for matrix multiplication
            bbox_world_h = np.hstack([bbox_world, np.ones((bbox_world.shape[0], 1))])  # Nx4

            # Transform from world to camera coordinates
            # cam_to_world_transform is 4x4 (world = cam_to_world @ cam)
            # So inverse will give cam = world_to_cam
            world_to_cam = np.linalg.inv(cam_to_world_transform)
            bbox_cam_h = (world_to_cam @ bbox_world_h.T).T  # Nx4

            # Drop homogeneous coordinate, keep Nx3
            bbox_cam = bbox_cam_h[:, :3]

            # Perspective projection to pixel coordinates
            u = fx * (bbox_cam[:, 0] / bbox_cam[:, 2]) + cx
            v = fy * (bbox_cam[:, 1] / bbox_cam[:, 2]) + cy

            bbox_cam[:, 0] = fx * (bbox_cam[:, 0] / bbox_cam[:, 2]) + cx
            bbox_cam[:, 1] = fy * (bbox_cam[:, 1] / bbox_cam[:, 2]) + cy

            # Check if bounding box is fully within image
            valid = True
            for u_pixel, v_pixel, _ in bbox_cam:
...
def sample_poses_for_frames(
    ...
    for j in range(n_frames):
        bproc.renderer.set_world_background(
            [np.random.normal(0.35, 0.05)] * 3, strength=1
        )
        sample_pose = sample_pose_func(
            j, image_width, image_height, cam_to_world_transform
        )
        ...
        bproc.camera.add_camera_pose(cam_to_world_transform, frame=j)
...
def render_frames() -> dict:
    ...
    bproc.renderer.enable_segmentation_output(
        map_by=["instance", "category_id", "position", "euler"],
        default_values={
            "category_id": 0,
...
def write_data(path: str, data: dict):
    ...
    data["depth"][data["depth"] > 10.0] = 0.0

    frame_max = data["depth"].max(axis=(1, 2), keepdims=True)
    noise = np.random.normal(0.1, 0.0025, size=data["depth"].shape)
    mask = data["depth"] == 0.0
    data["depth"][mask] = (frame_max + noise)[mask]

    bproc.postprocessing.add_kinect_azure_noise(
        data["depth"][0], missing_depth_darkness_thres=0
    )
    data["depth"] = (np.round(data["depth"] * 1000)).astype(np.uint16)

    # Data to store in the hdf5 file
    for frame in data["instance_attribute_maps"]:
        for instance_dict in frame:
            instance_dict["position"] = list(instance_dict["position"])
            instance_dict["euler"] = list(instance_dict["euler"])
...
if __name__ == "__main__":
    ...
    light.set_location(
        [
            cam_to_world_transform[0][3],
            cam_to_world_transform[1][3],
            cam_to_world_transform[3][3] + 1.0,
        ]
    )
