...
class Detection(Node):
    ...
    def detect_contours(self):
        if self.combined_img['rgb'] is not None and self.combined_img['depth'] is not None:
            ...
            # Blur the depth map and apply time filtering to stabilize the map
            img_depth_processed = cv2.medianBlur(img_depth, 3)
            self.prev_filtered_img['depth'] = img_depth_processed = utils.time_filter(img_depth_processed, self.prev_filtered_img['depth'], 0.2, (50)*(2**8)).copy()
            ...
            img_depth_processed = clahe.apply(img_depth_processed).astype(np.uint16)
            ...
            _, mask1 = cv2.threshold(img_depth_processed, 195, 255, cv2.THRESH_BINARY_INV)
            mask1 = mask1.astype(np.uint8)

            mask2 = np.zeros_like(img_depth_processed)
            _, mask2 = cv2.threshold(img_depth_processed, 120, 255, cv2.THRESH_BINARY)
            mask2 = mask2.astype(np.uint8)

            # Convert depth image to uint8 and append to the HSV image
            img_depth_processed = img_depth_processed.astype(np.uint8)
            ...
            contours_depth, _ = self.getContours(img_depth_processed, canny_range=(18, 18), dilation_kernel_size=(3, 3), dilation_iterations=1)
            ...
            #img_msg = self.bridge.cv2_to_imgmsg(img_processed3, encoding='passthrough')
    ...
    def drawDetectionResults(self, img_rgb: np.ndarray, img_depth: np.ndarray, contours: tuple):
        ...
        detection_mask = np.zeros((480, 640, 3), dtype=np.uint8)
        selected_contours = []
        for cnt in contours:
            depth = 0
            for point in cnt:
            ...
            if lim_low <= ratio <= lim_high:
                text = "Small"
            elif abs(ratio - 3.58) <= diff or abs(ratio - 1.36) <= diff:
                text = "Medium"
            else:
                text = ""

            if 2800 < cv2.contourArea(cnt) < 12500 and text:
                ...
                for cnt2 in selected_contours:
                    ...
                    cy = int(M['m01']/M['m00'])
                    if (
                        cnt2[1][0] <= cx <= cnt2[1][0] + cnt2[1][2] and
                        cnt2[1][1] <= cy <= cnt2[1][1] + cnt2[1][3]
                        ):
                        add = False
                        break
        ...
        return cv2.addWeighted(img_rgb, 0.9, detection_mask, 0.1, 0)
